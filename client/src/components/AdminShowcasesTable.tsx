...
export default function AdminShowcasesTable() {
  const { data, isLoading } = trpc.admin.getShowcases.useQuery();
  ...
  return (
    <Card className="bg-[#2a0a1a] border-[#d4af37] border-2">
      <CardHeader>
        <CardTitle className="text-[#d4af37]">Showcase Registrations ({data?.length || 0})</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-[#d4af37]">
                ...
                <TableHead className="text-[#d4af37]">Business Name</TableHead>
            ...
            <TableBody>
              {data?.map((item) => (
                <TableRow key={item.id} className="border-[#4a1a2a] hover:bg-[#4a1a2a]">
                  <TableCell className="text-white font-bold">{item.showcaseType}</TableCell>
                  <TableCell className="text-white">{item.fullName}</TableCell>
                  <TableCell className="text-white">{item.businessName || "-"}</TableCell>
              ...
              {data?.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-white py-4">No showcases found.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
